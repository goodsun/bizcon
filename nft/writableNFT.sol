...
contract writableNFT is ERC721Enumerable, RoyaltyStandard {
    ...
    function updateTokenURI(uint256 tokenId, string memory newMetaUrl) external {
        _requireMinted(tokenId);
        require(
            msg.sender == _owner || msg.sender == _creator || checkAdmin(),
            "Only owner, creator or admin can update token URI"
        );
        _metaUrl[tokenId] = newMetaUrl;
    }
